...
app.MapGet("/weather-report/{zip}", async (string zip, [FromQuery] int? days, IWeatherReportAggregator weatherReport) => {
    ...
    var report = await weatherReport.BuildReport(zip, days.Value); 
